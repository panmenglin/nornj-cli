...
function renderFile(path, pathTo, ...params) {
  ...
  let fileTxt = pathExists && fs.readFileSync(path, 'utf-8') ? nj.render(fs.readFileSync(path, 'utf-8'), ...params) : '';
  ...
  const pathToExists = fs.existsSync(pathTo);
  if (!isInsertTmpl || (isInsertTmpl && pathToExists)) {
